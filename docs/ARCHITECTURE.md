...
~/.claude/projects/**/*.jsonl   (Claude Code 가 실시간 기록)
     ...
     [FastAPI 70 routes]        main.py
      ...
      /   REST   WS    \
     /    |     |      \
  /  ← 공개 랜딩      /app  ← SPA 대시보드 (인증)
 /landing/ ← 공개 소개 (동일 파일 미러)
     [정적 프론트엔드]         static/*.js · landing-pages/*.html
```
...
| 그룹 | 라우트 | 역할 |
|---|---|---|
| 인증 | `/login`, `/api/auth/login`, `/logout`, `/me` | 쿠키 세션 인증 (HMAC 서명, rate limit 5/min/IP). 로그인 성공 시 `/app` 으로 이동 |
| 공개 | `/` (랜딩), `/features`, `/landing`, `/landing/*` | `/` 는 공개 랜딩(landing-pages/index.html), `/features` Feature Reference, `/landing/*` 는 `/` 와 동일 파일 미러 (모두 인증 우회) |
| SPA | `/app`, `/app/` | 대시보드 HTML shell (인증 필요). 예전 `/` 에서 2026-04-18 이동 |
...
- `auth`: `DASHBOARD_PASSWORD` 설정 시 쿠키 세션 검증 (`dash_session`). `/`, `/static/*`, `/landing/*`, `/api/health`, `/metrics`, `/api/ingest`, `/api/collector.py`, `/login`, `/features` 은 우회. `/app` 및 그 외 모든 경로는 인증 필요
