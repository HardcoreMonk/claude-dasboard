...
## Hook 설치 (실시간 timeline)

Claude Code 의 SessionStart / Stop / Notification hook 을 활성화하면 1s 내 push 로 timeline 뷰가 갱신된다 (JSONL 폴링은 5–15s 지연). 미설치 시에도 timeline 은 JSONL 파생 이벤트만으로 동작 — opt-in.

```bash
./.venv/bin/python install_hooks.py install
# 토큰 회전:
./.venv/bin/python install_hooks.py rotate-token --yes
# 제거:
./.venv/bin/python install_hooks.py uninstall
```

`~/.claude/settings.json` 에 hook 정의가 추가되고 `~/.claude/.hook-token` 이 자동 생성된다 (chmod 600). 토큰은 `Authorization: Bearer` 로 `POST /api/hooks/*` 인증에 사용. 설계는 [ADR-0007](docs/adr/0007-session-events-hooks.md).

## Timeline 뷰

세션 detail 페이지에서 `?view=timeline` 으로 진입 — toggle pill `[ Conversation | Timeline ]`. 카드 형태 + WebSocket realtime (`timeline_event` 채널) 으로 hook 이벤트가 즉시 그려진다. 8종 event_type (`session_start` / `session_stop` / `message_user` / `message_assistant` / `end_turn` / `tool_use` / `permission_prompt` / `subagent_dispatch`) 을 단일 append-only 로그 (`session_events`) 에서 읽어 렌더.
...
static/
  ...
  login.html         로그인 페이지 (Instrument Serif 히어로 + Iconify Solar + ambient orb + 한국어 UI)
...
| [`docs/adr/`](docs/adr/) | 7건 아키텍처 결정 기록 |
