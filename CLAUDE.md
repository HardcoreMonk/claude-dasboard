...
static/app.js       2768줄  core: state/ws(safeFetch 재시도)/routing/utils/modals + h() + 4단계 상태 감지 (idle/tool/subagent) + Web Audio chime + persistent error banner
...
static/app.css       384줄  스타일 + 라이트모드 전체 매핑 (WCAG AA 4.5:1 검증 완료) + 반응형
...
- 캐시버스팅: **파일명 기반** — `/static/app.vN.js` / `app.vN.css`. `index.html` 의 `.vN` 을 일괄 bump (현재 v=59). 서버의 `/static/{path:path}` 라우트가 정규식으로 `.vN` 을 strip 해 실제 파일을 서빙한다.
