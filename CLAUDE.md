...
```
main.py             2147줄  FastAPI 47 routes + /metrics + WS (per-conn lock) + summarize_preview + _iso_to_epoch
...
# Frontend — 8 파일 모듈화
static/index.html    877줄  Tailwind 쉘 + drawer preview 패널 + idle notify 설정 + Charts Row 3
static/app.js       2758줄  core: state/ws(safeFetch 재시도)/routing/utils/modals + h() + idle notify (batch-deferred, subagent-aware) + Web Audio chime + persistent error banner
static/sessions.js   543줄  sessions domain: load, filters, presets, bulk, mgmt, $/hr, 모바일 hide-sm
static/overview.js   336줄  hero/chips/forecast/top5 + slide drawer + active-first sort + 캐시 사실 표시
...
static/app.css       378줄  스타일 + 라이트모드 전체 매핑 (WCAG AA 4.5:1 검증 완료) + 반응형
...
- broadcast record 에 `is_subagent` 플래그 포함 → 프론트에서 subagent `end_turn` 을 idle 로 처리하지 않음.
...
- 캐시버스팅: **파일명 기반** — `/static/app.vN.js` / `app.vN.css`. `index.html` 의 `.vN` 을 일괄 bump (현재 v=58). 서버의 `/static/{path:path}` 라우트가 정규식으로 `.vN` 을 strip 해 실제 파일을 서빙한다.
