...
static/app.js       2810줄  core: state/ws(safeFetch dedup+재시도)/routing/utils/modals + h() + 4단계 상태 감지 + Web Audio chime + persistent error banner
...
- 캐시버스팅: **파일명 기반** — `/static/app.vN.js` / `app.vN.css`. `index.html` 의 `.vN` 을 일괄 bump (현재 v=63). 서버의 `/static/{path:path}` 라우트가 정규식으로 `.vN` 을 strip 해 실제 파일을 서빙한다.
